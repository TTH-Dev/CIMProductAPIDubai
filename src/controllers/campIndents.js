...
export const createBilling = catchAsync(async (req, res, next) => {
  const { patientId, billDetails } = req.body;

  if (!patientId) {
    return next(new AppError("PatientId is required", 400));
  }

  const manualBilling = await CampBilling.create(req.body);
...
export const getManualBillById=catchAsync(async(req,res)=>{
    ...
    const bill=await CampBilling.findById(id).populate("productId").populate("patientId")
...
export const getAllBillings = catchAsync(async (req, res) => {
  ...
  const features = new APIFeatures(CampBilling.find(filter), req.query)
    .filter()
    .sort()
    .limitFields();
